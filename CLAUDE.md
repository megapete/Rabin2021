...
- **`SimulationModel.swift`** (`actor SimulationModel`) — the transient solver. Built from a `PhaseModel`, it integrates the coupled dV/dt, dI/dt system with an adaptive **RK45** method (`SimulateRK45` → `DifferentialFormula`), driven by an input impulse `WaveForm`, and produces `[SimulationStepResult]`. `DoSimulate` runs **two** RK45 passes (the first to derive the real fundamental frequencies, the second to use them), and both `DoSimulate` and `SimulateRK45` accept an optional progress continuation — see *Progress reporting* below. Both return an **empty array for cancellation as well as failure**, so callers must check `Task.isCancelled` to tell the two apart.
